# # C:\Users\arnab\OneDrive\Desktop\AIOS-LSFS_local\aios\llm_core\adapter.py
...
#             api_key = config.get("api_key") # Fetch API Key from config dict
...
#             # --- ADD THIS NEW BLOCK FOR OPENAI/KRUTRIM ---
#             case "openai":
#                 # Check if a custom hostname/base_url is provided (e.g., for Krutrim)
#                 base_url = config.hostname # In LLMConfig, hostname holds the base_url
                
#                 # Get the API key specifically for this provider
#                 # (You might need to import config from config_manager if not available here, 
#                 # but usually it's set in env vars by _setup_api_keys)
                
#                 # If using Krutrim via OpenAI backend:
#                 # if base_url and "olakrutrim" in base_url:
#                 #     #  from aios.config.config_manager import config as global_config
#                 #      api_key = global_config.get_api_key("krutrim") 
...
#                      # If using Krutrim via OpenAI backend:
#                 if base_url and "olakrutrim" in base_url:
#                     # Use 'config' which is imported at the top of the file
#                     api_key = config.get_api_key("krutrim") 
...
#                     ))
#                 else:
#                     # Fallback for standard OpenAI
#                     self.llms.append(config.backend + "/" + config.name)
#             # ---------------------------------------------
#             case "vllm":
#                 self.llms.append(OpenAI(
...
#             api_key = self.llm_configs[model_idx].get("api_key", None)
...
import re
import os
from aios.config.config_manager import config as global_system_config 
from dataclasses import dataclass
import logging
from typing import Any
...
class LLMAdapter:
    ...
    def _setup_api_keys(self) -> None:
        ...
        for provider, env_var in api_providers.items():
            api_key = global_system_config.get_api_key(provider)
            if api_key:
                if provider == "huggingface":
                    os.environ["HF_TOKEN"] = api_key.get("auth_token", "")
                    os.environ["HF_HOME"] = api_key.get("home", "")
    ...
    def _initialize_llms(self) -> None:
        for config_item in self.llm_configs:
            llm_name = config_item.get("name")
            llm_backend = config_item.get("backend")
            max_gpu_memory = config_item.get("max_gpu_memory")
            eval_device = config_item.get("eval_device")
            hostname = config_item.get("hostname")
            api_key = config_item.get("api_key") 
        ...
        match config.backend:
            ...
            case "openai":
                ...
                if base_url and "olakrutrim" in base_url:
                    ...
                    if not api_key:
                         api_key = global_system_config.get_api_key("krutrim") 
    ...
    def _handle_completion_error(self, error: Exception) -> LLMResponse:
        ...
        if "API key provided:" in error_msg:
            key_start = error_msg.find("API key provided:") + len("API key provided: ")
            key_end = error_msg.find(".", key_start)
            if key_end == -1: key_end = error_msg.find(" ", key_start)
            if key_end != -1:
                api_key = error_msg[key_start:key_end]
                masked_key = f"{api_key[:2]}****{api_key[-2:]}" if len(api_key) > 4 else "****"
                error_msg = error_msg[:key_start] + masked_key + error_msg[key_end:]
        ...
        if "Invalid API key" in error_msg or "API key not found" in error_msg:
            return LLMResponse(response_message="Error: Invalid API key.", error=error_msg, finished=True, status_code=402)
        return LLMResponse(response_message=f"LLM Error: {error_msg}", error=error_msg, finished=True, status_code=500)
    ...
    def execute_llm_syscall(self, llm_syscall, temperature: float = 0.0) -> LLMResponse:
        try:
            ...
            return LLMResponse(response_message=f"System Error: {str(e)}", error=str(e), finished=True, status_code=500)

    def _prepare_messages(self, llm_syscall, model, messages: List[Dict], tools: Optional[List] = None) -> List[Dict]:
        if self.context_manager:
            pid = llm_syscall.get_pid()
            restored_context = self.context_manager.load_context(pid=pid, model=model)
            if restored_context:
                if messages[-1]["role"] != "assistant":
    ...
    def _get_model_response(
        ...
        # --- 1. DETECT KRUTRIM TO FORCE MANUAL TOOLS ---
        is_krutrim = False
        if isinstance(model, OpenAI) and "olakrutrim" in str(model.base_url):
            is_krutrim = True
        
        # For Krutrim, we MUST convert native tools to text instructions 
        # to avoid the "400 auto tool choice" error.
        manual_tools_prompt = False
        if is_krutrim and tools:
            # Modify messages to include tool definitions in text
            messages = tool_calling_input_format(messages, tools)
            manual_tools_prompt = True
            # HIDE tools from the native API call
            tools = None 

        # --- Standard Context Manager Logic ---
        if self.use_context_manager:
            pid = llm_syscall.get_pid()
            time_limit = llm_syscall.get_time_limit()
            completed_response, finished = self.context_manager.save_context(
            ...
            if finished: self.context_manager.clear_context(pid)
            return completed_response, finished
        
        # --- 2. Build kwargs ---
        completion_kwargs = {"messages": messages, "temperature": temperature}
        
        # Only add native tools if we are NOT in manual mode
        if tools and not manual_tools_prompt:
            completion_kwargs["tools"] = tools
        
        if message_return_type == "json":
            completion_kwargs["format"] = "json"
            if response_format: completion_kwargs["response_format"] = response_format
        
        # --- 3. Execute Model ---
        if isinstance(model, str):
            if api_base: completion_kwargs["api_base"] = api_base
            if api_key: completion_kwargs["api_key"] = api_key
            completed_response = completion(model=model, **completion_kwargs)
            ...
            completed_response = model.chat.completions.create(model=model_name, **completion_kwargs)
            
            # If we used native tools (not Krutrim), decode normally
            if tools:
                completed_response = decode_litellm_tool_calls(completed_response)
                return completed_response, True
            
            # If we used MANUAL tools (Krutrim), we must parse the text response
            content = completed_response.choices[0].message.content
            if manual_tools_prompt:
                # Attempt to extract JSON tool calls from the text response
                # We use the utility from aios.utils
                parsed_tools = parse_tool_calls(content)
                if parsed_tools:
                    # Return formatted tool calls so AIOS Scheduler understands
                    return parsed_tools, True
    ...
    def _process_response(self, completed_response: str | List, finished: bool, tools: Optional[List] = None, message_return_type: Optional[str] = None) -> LLMResponse:
        # If response is a list (likely tool calls), process it
        if isinstance(completed_response, list):
             tool_calls = double_underscore_to_slash(completed_response)
             return LLMResponse(response_message=None, tool_calls=tool_calls, finished=finished)

        # Fallback for standard text
        if tools and isinstance(completed_response, str):
             # Double check if we missed parsing it earlier
             # (Sometimes parse_tool_calls is called here in other implementations)
             pass
